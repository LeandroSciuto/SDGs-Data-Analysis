...
### Inserisco la media in base ai gruppi

# GDPc
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(GDPc = ifelse(is.na(GDPc), mean(GDPc, na.rm =TRUE), GDPc))

# Infant mortality rate
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(`Infant mortality rate` = ifelse(is.na(`Infant mortality rate`), mean(`Infant mortality rate`, na.rm =TRUE),
                                          `Infant mortality rate`))
# Health GDP
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(`Health GDP` = ifelse(is.na(`Health GDP`), mean(`Health GDP`, na.rm =TRUE),
                               `Health GDP`))
# Physician rate
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(`Physicians rate` = ifelse(is.na(`Physicians rate`), mean(`Physicians rate`, na.rm =TRUE),
                                    `Physicians rate`))
# Education GDP
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(`Education GDP` = ifelse(is.na(`Education GDP`), mean(`Education GDP`, na.rm =TRUE),
                                  `Education GDP`))
# Threatned species
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(`Threatned species` = ifelse(is.na(`Threatned species`), mean(`Threatned species`, na.rm =TRUE),
                                      `Threatned species`))
# CO2em
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(CO2em = ifelse(is.na(CO2em), mean(CO2em, na.rm =TRUE),
                        CO2em))
# Quality of water
def_data = def_data %>% 
  group_by(gruppi) %>% 
  mutate(`Quality of water` = ifelse(is.na(`Quality of water`), mean(`Quality of water`, na.rm =TRUE),
                                     `Quality of water`))
